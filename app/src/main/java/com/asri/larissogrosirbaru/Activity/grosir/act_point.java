...
public class act_point extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_act_point);
        ...
        session = new Session(act_point.this);
        ...
        getSettingVoucher.enqueue(new Callback<BaseResponse<SettingVoucher>>() {
            ...
            public void onResponse(Call<BaseResponse<SettingVoucher>> call, final Response<BaseResponse<SettingVoucher>> response) {
                if (response.isSuccessful()) {
                    ...
                    adapterRedeem = new AdapterRedeem(act_point.this, nmr, nama_voucher, nilai_voucher, ketentuan, masa_berlaku, new AdapterRedeem.OnEditLocationListener() {
                        @Override
                        public void onClickAdapter(final int position) {
                            if (point < Integer.parseInt(ketentuan.get(position))) {
                                Toasty.warning(act_point.this, "Maaf Point Anda Kurang !!!", Toast.LENGTH_SHORT).show();
                            } else {
                                final SweetAlertDialog pDialog = new SweetAlertDialog(act_point.this, SweetAlertDialog.WARNING_TYPE);
                                        ...
                                        tambahVoucher.enqueue(new Callback<BaseResponse>() {
                                            ...
                                            public void onResponse(Call<BaseResponse> call, Response<BaseResponse> response) {
                                                if (response.isSuccessful()) {
                                                    ...
                                                    dataPoinVoucher();
                                                    dataSettingVoucher();
                                                    Toasty.success(act_point.this, "Voucher Berhasil Anda klaim !!!", Toast.LENGTH_SHORT).show();
                                                } else {
                                                    Toasty.warning(act_point.this, "Error, voucher gagal di klaim", Toast.LENGTH_SHORT).show();
                                                }
                                            }

                                            @Override
                                            public void onFailure(Call<BaseResponse> call, Throwable t) {
                                                Toasty.error(act_point.this, "Error "+t.getMessage(), Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    }
                                });
...
//                                System.out.println(dt+" | "+formatter.format(date));
    ...
    public void zoomBarcode() {
        final Dialog dialog = new Dialog(act_point.this);
